import re
with open('apps/frontend/src/app/live/page.tsx', 'r', encoding='utf-8') as f:
    content = f.read()
...
# Find the old effect to replace
...
content = content.replace(old_effect, suggestions_effect)

# Find the Title area and replace it with Title + Search area
old_title_area = '''        {/* Title Area */}
...
new_title_area = '''        {/* Title & Search Area */}
        <div className="text-center mt-2 mb-8 relative z-10 animate-fade-in px-4">
          <p className="text-[10px] font-bold tracking-[0.3em] uppercase text-primary mb-2 flex items-center justify-center gap-2">
            <span className="text-orange-500">?</span> {activeBhajan.deity} BHAJAN
          </p>
...
# Now apply scroll handlers to the lyrics scroll area
old_scroll_div = '''        {/* Lyrics Scroll Area */}
        <div 
          className="relative z-10 flex-1 overflow-y-auto no-scrollbar px-6 scroll-smooth pb-32"
          
        >'''

new_scroll_div = '''        {/* Lyrics Scroll Area */}
        <div 
...
# Now inject the random suggestions at the bottom of the lyrics container
old_bottom = '''                );
              })}
            </div>
          </div>'''
...
with open('apps/frontend/src/app/live/page.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
